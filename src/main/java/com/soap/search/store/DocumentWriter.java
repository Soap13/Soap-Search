...
//                }
     ...
     * @param docList
     */
    public void writeTermOffSetGolomb2(List<Document> docList) throws IOException, InvocationTargetException, IllegalAccessException {
        long startTime = System.currentTimeMillis(); // 记录开始时间
        Log.info("词位置开始提取...");
        Map<String, ArrayList<Integer>> termMap = null;
        Map<String,Map<String,List<Integer>>>termOffset=new HashMap<>();
...
//                 }
        ...
        Log.info("词位置开始写入...");
        Set<String> termSet=termOffset.keySet();
        ...
        //for(Map.Entry<String,Map<String,List<Integer>>> en:termOffset.entrySet()){
            ...
            PrefixCompression.CompressedTerm pTerm=compressedList.get(i);
            outputDF.writeVInt(pTerm.prefixLength);
            outputDF.writeString(pTerm.suffix);
//            outputDF.writeString(term);//词
