...
public class SkipList<K extends Comparable<K>, V> {
    ...
    private static class Node<K extends Comparable<K>, V> {
        ...
        Node(K key, V value, int level) {
            ...
            this.value = value;
            this.level = level;
            this.forwards = new ArrayList<>(Collections.<Node<K, V>>nCopies(level + 1, null));
        }
    ...
    public synchronized boolean insertNode(K key, V value) {
        ...
        ArrayList<Node<K, V>> update = new ArrayList<>(Collections.<Node<K, V>>nCopies(MAX_LEVEL + 1, null));
